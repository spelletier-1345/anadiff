...
.defineArrays <- function(dirName, fileOut, fileArray) {
  ...
  # Returns:
  #   le tableau arrays
  fileListe <- list.files(path = ".", pattern = ("_[[:digit:]]+_[[:digit:]]+.txt$"))
  arrays <- read.table(fileArray, header=T, encoding="utf-8")
...
.exportQCArray <- function(chip, smoothScatterRG, smoothScatterMA, histRG, histMA,
                          spikesPlot, spikesTab, greenLoc, redLoc, diffLoc, labelling, dirName) {
  graphics.off()
...
.plotSpikes <- function(spikesTab, indNbCol, nbCol, minSpikes, maxSpikes) {
  # Graph Spikes
...
.RG4Graph <- function(RG, labelling, dirName) {
  ...
  for (indNbCol in seq(1,nbCol)) {
    .exportQCArray(chip=RG$targets$FileName[indNbCol],
                   ...
                   labelling=labelling, dirName=dirName)
  }
  return(MA)
}
...
.selectionDossier <- function(message) {
...
.tabSpikes <- function(spikesTab, indNbCol, nbCol) {
