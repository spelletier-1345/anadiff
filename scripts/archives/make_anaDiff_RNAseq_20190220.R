...
.anaDiffRNAseq <- function(){
  ...
  pathResults <- .mkdirAnaDiff(pathFolder)
  ...
  pdfplots <- vector((length(comparisons)*2), mode='list')
  i.pdf <- 0
  ...
  for (comparison in comparisons) {
    ...
    pdfplots <- .functionEdgeR(repet, countTable, group,comparison, fileOut, pathResults, pdfplots, i.pdf)
    pdfplots <- .functionDESeq2(myCompare, countTable, comparison, fileOut, pathResults, pdfplots, i.pdf)
    i.pdf <- i.pdf+2
  }
  graphics.off()
  pdf(paste(pathResults, "QC_plots_", Sys.Date(), ".pdf", sep=""), onefile=TRUE)
  for (pdfplot in pdfplots) {
    replayPlot(pdfplot)
  }
  graphics.off()
  return("OK")
...
.functionDESeq2 <- function(myCompare, countTable, comparison, fileOut, pathResults, pdfplots, i.pdf) { 
...
.functionEdgeR <- function(repet, countTable, group, comparison, fileOut, pathResults, pdfplots, i.pdf) {
...
.mkdirAnaDiff <- function(pathFolder) {
  ...
  dirName <- paste(pathFolder, "AnaDiff", sep="")
